  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/5 left-0 w-6 h-6 bg-blue-400/20 dark:bg-blue-600/15 rounded-full blur-sm" 
           style={{ 
             animation: 'waveMotion 16s ease-in-out infinite',
             animationDelay: '0s'
           }}></div>
      <div className="absolute bottom-1/5 right-0 w-8 h-8 bg-cyan-400/20 dark:bg-cyan-600/15 rounded-full blur-sm" 
           style={{ 
             animation: 'waveMotion 14s ease-in-out infinite reverse',
             animationDelay: '3s'
           }}></div>
      <div className="absolute top-1/2 left-1/4 w-5 h-5 bg-indigo-400/20 dark:bg-indigo-600/15 rounded-full blur-sm" 
           style={{ 
             animation: 'circularMotion 18s linear infinite',
             animationDelay: '1s'
           }}></div>
              ...
              {skills.technical.map((skill, index) => (
                <div key={index} className="group cursor-pointer">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden group-hover:h-4 transition-all duration-300">
                    <div 
                      className={`h-full bg-gradient-to-r from-blue-500 to-cyan-600 rounded-full transition-all duration-1000 ease-out group-hover:shadow-lg ${
          ...
          <div className={`backdrop-blur-xl bg-white/15 dark:bg-gray-800/15 p-8 rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/20 hover:shadow-3xl hover:scale-[1.02] hover:bg-white/25 dark:hover:bg-gray-800/25 transition-all duration-500 ${
            ...
            <div className="grid grid-cols-1 gap-4">
              {skills.expertise.map((item, index) => (
                // --- START OF CHANGE ---
                // Outer div now handles the staggered entrance animation
                <div
                  key={index}
                ...
                >
                  {/* Inner div handles styling and hover effects without a delay */}
                  <div 
                    className="flex items-center shadow-md gap-3 p-4 bg-gradient-to-r from-blue-50/50 to-cyan-50/50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-xl hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer group backdrop-blur-sm"
                  >
