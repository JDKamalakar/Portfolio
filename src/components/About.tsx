    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          <div className={`backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 p-8 rounded-lg shadow-2xl border border-white/20 dark:border-gray-700/20 hover:scale-[1.02] transition-all duration-500 group ${hasMoreContent ? 'cursor-pointer' : 'cursor-default'} ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
            } ${isExpanded ? 'scale-[1.02] shadow-3xl bg-white/30 dark:bg-gray-800/30' : ''} theme-glow`} onClick={() => hasMoreContent && setIsExpanded(!isExpanded)}>
            <div className="relative">
              <div className={`overflow-hidden transition-all duration-700 ease-in-out ${isExpanded ? 'max-h-96' : 'max-h-34'
                }`}>
                <div className={`transform transition-all duration-500 ${isExpanded ? 'translate-y-0 scale-100' : 'translate-y-0 scale-100'
                  }`}>
            ...
            <div className="space-y-4 mt-6">
              {about.highlights.map((highlight, index) => (
                <div
                  key={index}
                  className={`flex items-center gap-3 hover:translate-x-2 transition-all duration-300 cursor-default group/item ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'
                    }`}
                  style={{ transitionDelay: `${index * 100}ms` }}
                >
                  <div className={`w-2 h-2 ${index === 0 ? 'bg-blue-500' : index === 1 ? 'bg-cyan-500' : 'bg-indigo-500'
          ...
          <div className={`space-y-6 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'
            } transition-all duration-1000 delay-300`}>
            <div className="backdrop-blur-xl bg-white/20 dark:bg-gray-800/20 p-6 rounded-xl shadow-2xl border border-white/20 dark:border-gray-700/20 hover:scale-[1.02] hover:bg-white/30 dark:hover:bg-gray-800/30 transition-all duration-300 group theme-glow">
              ...
              <div className="flex flex-wrap gap-2">
                {about.languages.map((lang, index) => (
                  ...
                  <span
                    key={lang}
                    className="relative isolate px-3 py-1 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-lg text-sm hover:scale-110 theme-glow transition-all duration-300 cursor-pointer overflow-hidden"
                  ...
                  if (link) {
                    return (
                      <a
                        ...
                        // MODIFIED: Restructured tag for guaranteed transparency
                        className="relative isolate px-3 py-1 bg-gradient-to-r from-cyan-500 to-indigo-600 text-white rounded-lg text-sm hover:scale-110 theme-glow transition-all duration-300 cursor-pointer overflow-hidden"
                        style={{ animationDelay: `${index * 50}ms` }}
                      >
